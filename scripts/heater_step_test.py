...
# Allow running as `python scripts/heater_step_test.py`.
...
def run_step_test(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        psu.set_voltage(0.0)
        # Some units need a short settle + occasional retry before OUTP state latches.
        output_on = False
        ...
        if not output_on:
            detail = ""
            try:
                info = psu.measure_all_info()
                detail = (
                    f" (mode={info['mode']}, "
                    f"ovp_fault={info['ovp_fault']}, "
                    f"ocp_fault={info['ocp_fault']}, "
                    f"otp_fault={info['otp_fault']})"
                )
            except Exception:
                pass
            raise RuntimeError(
                "PSU output did not turn on after 3 OUTP ON attempts."
                + detail
